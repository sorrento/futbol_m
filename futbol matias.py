...
# primero actualizar el excel que lee de las página web
...
ran=acct.groupby('Local').agg({'dif':'mean', 'vic':'mean', 'ptos':'sum'}).sort_values(['ptos', 'dif'], ascending=False)
...
ptos_rival=acct[['Local','Visita']].merge(ran.reset_index()[['Local', 'ptos']].rename(columns={'Local':'Visita'}),
                               on='Visita')
ptos_rival=ptos_rival.groupby('Local').mean().sort_values('ptos', ascending=False).rename(columns={'ptos':'ptos_rival'})
...
# Matriz# con nombres con ranking (para pintar la matriz por el mismo orden)
acct['Local_ran']=[dic_names[x] for x in acct.Local]
acct['Visita_ran']=[dic_names[x] for x in acct.Visita]

# acct.to_csv('matriz.csv', sep=';')
